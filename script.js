...
navigator.geolocation.getCurrentPosition(
  ...
  function (position) {
    ...
    L.marker(cordinates).addTo(map).bindPopup("Your Location").openPopup();
    ...
    map.on("click", function (ev) {
      ...
      Distance = (
        ev.target.distance(cordinates, clickedLocation) / 1000
      ).toFixed(2);
      DistanceAll.textContent = `Distance: ${Distance}miles`;
      let averageWalkingPaceAirLine = 2.5;
      let averageBicycleAirLine = 13;
      let averageCarSpeedAirLine = 20;

      console.log((Distance / 10) * 60);
      console.log((Distance / 2.5) * 60);
      TimeBicycle =
        (Distance / 13) * 60 < 60
          ? `Distance on time   Bicycle: ${((Distance / 13) * 60).toFixed(
              2
            )} minutes`
          : `Distance on time   Bicycle: ${(Distance / 13).toFixed(2)} hours`;
      TimeCar =
        (Distance / 20) * 60 < 60
          ? `Distance on time   Car: ${((Distance / 20) * 60).toFixed(
              2
            )} minutes`
          : `Distance on time   Car: ${(Distance / 20).toFixed(2)} hours`;
      TimeWalk =
        (Distance / 2.5) * 60 < 60
          ? `Distance on time   Walk: ${((Distance / 2.5) * 60).toFixed(
              2
            )} minutes`
          : `Distance on time   Walk: ${(Distance / 2.5).toFixed(2)} hours`;

      DistanceWalking.textContent = TimeBicycle;
      DistanceCar.textContent = TimeCar;
      DistanceBicycle.textContent = TimeWalk;
    });
  },
  //Rejected Location
  function () {
...
function results() {
  ...
  if (
    (imageUser.src === rock && imageComp.src === scissor) ||
    (imageUser.src === scissor && imageComp.src === paper) ||
    (imageUser.src === paper && imageComp.src === rock)
  ) {
    resultsH1.textContent = "You Won";
    resultsH1.style.color = "green";
  }
  if (
    (imageUser.src === scissor && imageComp.src === rock) ||
    (imageUser.src === paper && imageComp.src === scissor) ||
    (imageUser.src === rock && imageComp.src === paper)
  ) {
